...
The active compute profile controls the Archivist runtime. Under `muse-max`, invoke Archivist through `python3 ~/.codex/codex_workflow/runtime/muse_worker.py --role archivist --workspace <project-root> --task-file <capsule-file> --logical-worker-id <archivist-id> --caller-scope <opaque-scope>`; each turn is a bounded Muse Spark 1.3 Contributor / max invocation and follow-up on the same assignment may use `--resume` when its binding is still safe. Do not use `agent_type`, `fork_turns`, `resume_agent`, or `wait_agent` for that profile. Under Codex-backed profiles, use the internal lifecycle below.
...
For `muse-max`, start closure with a fresh logical `archivist` worker/session. Give it the verified canonical deployment-state references plus only the documentation scope and evidence it needs. If that same closure assignment needs a bounded follow-up, resume its exact session when safe rather than creating a replacement implicitly.
