...
| Role | Capsule |
| --- | --- |
| Explorer | **Project Context Scope**; **Context Task + Goal**; **Main-Agent Context Guidance** |
| Investigator | **Problem ID**; **Investigation Context**; **Evidence Question + Goal**; **Main-Agent Investigation Guidance** |
...
For Explorer, give the bounded project surface and context outcome needed for project-context discovery, mapping, or evidence retrieval. Explorer is disposable and read-only; do not turn it into persistent session context ownership or assign fault/solution/feasibility/prior-art research to it.

For Investigator, first define one bounded qualifying problem and assign one shared **Problem ID**. Create exactly three independent Investigator lanes for that Problem ID, each with a distinct **Task ID** and complementary evidence/search angle. Each lane may inspect bounded project evidence, Internet sources, or both as its assignment requires. Lanes do not coordinate, message one another, or vote. Each returns evidence, uncertainty, disagreement, and implications directly to Main. Main compares all three reports and makes the root-cause, architecture, solution, integration, and acceptance decisions; majority voting is not a decision rule.

If all three lanes cannot run concurrently, preserve the exact-three requirement and execute the missing lane when capacity becomes available. Do not reduce the lane count merely because capacity is temporarily smaller.

For Executors, give enough transferred project knowledge to complete the bounded package well, but leave local discovery, command selection, implementation, self-check, and ordinary repair to the worker. Give Senior the unresolved hard-decision context when solving it is the assignment.
...
Ask every worker for the smallest complete, evidence-linked, decision-ready return containing outcome, material changes or findings, verification evidence, limitations, residual risk, and only decisions Main must make. Reference bulky logs or diffs rather than reproducing them.
...
For an independent review, create a new Tester worker in the active worker runtime that did not perform the implementation being reviewed. Give it the exact target/base identity, acceptance intent, relevant durable evidence, and minimal required context. The implementing worker must not substitute for the independent reviewer. If review exposes a bounded problem that qualifies for Investigator, use the exact three-lane Investigator contract above.
