...
The current version is `1.1.17-private.7`, based directly on upstream `v1.1.17`
...
This release adds the global `luna-xhigh` compute profile. Micro, Default,
Tester, Companion, Investigator, and Archivist use GPT-5.6 Luna / xhigh while
Senior Executor remains GPT-5.6 Sol / medium. Main-agent selection is unchanged.
To keep the profile literal, Micro does not use the optional Spark override while
`luna-xhigh` is active.
...
python3 -B scripts/package_release.py --verify /absolute/path/to/fresh-output/codex_workflow-1.1.17-private.7.zip
```

Expected release assets:

- `codex_workflow-1.1.17-private.7.zip`
- `SHA256SUMS`
...
publishes a prerelease for tag `v1.1.17-private.7` with exactly the expected ZIP
