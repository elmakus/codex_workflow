...
def _test_private_version_and_user_marker_are_synchronized(self: unittest.TestCase) -> None:
    ...
    self.assertEqual(version, "1.1.17-private.13")
    ...
    self.assertGreater(base.parse_semver("1.1.17-private.13"), base.parse_semver("1.1.17-private.12"))
    self.assertEqual(base.NEXT_PACKAGE_VERSION, "1.1.17-private.14")
    ...
    def test_update_preserves_selected_muse_max_profile(self) -> None:
        ...
        user_agents.write_text(
            user_agents.read_text(encoding="utf-8").replace(
                "1.1.17-private.13", next_version
            ),
            encoding="utf-8",
        )
