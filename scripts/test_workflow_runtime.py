...
def _test_current_private_contract(self: unittest.TestCase) -> None:
    ...
    self.assertIn("When the active profile is `plus` or `muse-native`", heavy)
    ...
    self.assertIn("Under `plus` or `muse-native`, the standing internal-Codex-worker material-event policy", delegation)
